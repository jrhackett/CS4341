...
public class OurPlayerJH extends Player {

	int maxDepth;
	
	//OurPlayer is just our implementation of the Player class
	public OurPlayerJH(String n, int t, int l) {
		super(n, t, l);
	...
	public Move getMove(StateTree state) {
		...
		OurStateTreeJH betterState = new OurStateTreeJH(state.rows, state.columns, state.winNumber, state.turn, state.pop1, state.pop2, state.parent, state.boardMatrix);
		...
		for(OurStateTreeJH s : betterState.childrenStates) {
			if(s.bestValue == move) {
	...
	private int minimax(OurStateTreeJH state, int depth, int alpha, int beta) {
		...
		if(state.turn == 1) {
			...
			for(OurStateTreeJH s : state.childrenStates) {
